...
def get_average_sunshine_hours(entries: List[Tuple]) -> float:
    """
    Gibt die durchschnittliche Anzahl an Sonnenstunden aller in der Liste entries enthaltenen Monatseinträge
    (als float) zurück. Das Ergebnis wird dabei auf 3 Nachkommastellen gerundet.

    :param entries: Liste zu analysierende Monatseinträge
    :return: Durchschittliche Anzahl an Sonnenstunden aller Einträge
    """
...
def number_of_months_with_min_sunshine_hours(
    ...
    """
    Gibt die Anzahl der Monatseinträge in der Liste entries zurück in denen die durchschnittliche Anzahl an
    Sonnenstunden mindestens num_hours betrug.

    :param entries: Liste zu analysierende Monatseinträge
    :param num_hours: Mindestanzahl an Sonnenstunden
    :return: Anzahl der Monatseinträge in denen die durchschnittliche Sonnenstundenanzahl mindestens num_hours betrug
    """
...
def get_average_sunshine_hours_per_year(entries: List[Tuple]) -> Dict[int, float]:
    ...
    for row in sorted_data:
        try:
            ...
            month_checker[row[0].year][idx_by_month] = row[0].month
