...
def softmax(x: np.array):
    exp_vectorised = np.vectorize(lambda z: math.exp(z))
    exps = exp_vectorised(x)
...
class FeedforwardNetwork:
    ...
    def forward(self, x: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        **Softmax-Funktion** genutzt werden.
        ...
        """
        not_activated: np.ndarray = np.matmul(x, self.weights)
        not_activated_with_bias: np.ndarray = np.apply_along_axis(lambda w: w + self.bias, 1, not_activated)
        activated = np.apply_along_axis(func1d=softmax, axis=1, arr=not_activated_with_bias)
        return not_activated_with_bias, activated
    ...
    def predict_labels(self, p: np.ndarray) -> np.ndarray:
        ...
        """
        return np.apply_along_axis(lambda x: np.where(x == max(x))[0][0], 1, p)
    ...
    def loss(self, y: np.ndarray, p: np.ndarray) -> np.ndarray:
        ...
        """
        n: int = y.shape[0]
        loss: np.ndarray[Double] = np.zeros((n, 1))
        for i in range(n):
            loss[i,0] = np.sum(np.log(p[i,]) * (-1) * y[i,])
        return loss
    ...
    def backward(self, x: np.ndarray, p: np.ndarray, y: np.ndarray):
        ...
        """
        grad_weights = -np.matmul(np.transpose(x), y - p)
        grad_bias = -np.sum(y - p, axis=0)
    ...
    def fit(self, x: np.ndarray, y: np.ndarray, epochs: int, batch_size: int, learning_rate: float):
        ...
        for i in range(epochs):
            ...
            for b_x, b_y in zip(x_batches, y_batches):
                ...
                #print("Durchschnittliche gewichte:", np.mean(self.weights))
                #print("Grad weights:", grad_weights[:20])
