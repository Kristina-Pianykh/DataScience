...
class FeedforwardNetwork:
    ...
    def forward(self, x: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        **Softmax-Funktion** genutzt werden.
        ...
        """
        not_activated = np.matmult(self.weights.T, x.T) + self.bias
    ...
    def predict_labels(self, p: np.ndarray) -> np.ndarray:
        ...
        """
        inputs = p.shape[0]
        predictions = np.zeros(inputs, dtype=np.float64)
        for idx in range(len(inputs)):
            predictions[idx] = np.argmax(inputs[idx])
