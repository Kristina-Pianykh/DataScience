...
class FeedforwardNetwork:
    ...
    def loss(self, y: np.ndarray, p: np.ndarray) -> np.ndarray:
        ...
        loss = np.zeros((p.shape[0], 10), dtype="float32")
        ...
        for row in range(log_transformation.shape[0]):
            entry = log_transformation[row]
            loss[row] = -1 * np.dot(log_transformation[row], y)
        return loss
