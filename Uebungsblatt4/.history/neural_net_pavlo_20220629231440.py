...
class FeedforwardNetwork:
    ...
    def forward(self, x: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        **Softmax-Funktion** genutzt werden.
        ...
        not_activated_with_bias: np.ndarray = np.apply_along_axis(
            lambda w: w + self.bias, 1, not_activated
        )
        activated = np.apply_along_axis(
            func1d=softmax, axis=1, arr=not_activated_with_bias
        )
