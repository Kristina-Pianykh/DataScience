    ...
    def forward(self, x: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        **Softmax-Funktion** genutzt werden.
        ...
        for i in range(num_iters):
            x1 = hypothesis(self.weights, x, self.bias)

            grad = (1 / m) * np.matmul(X.T, (h_w - y))
            w = w - alpha * grad

            L, _ = costFunction(w, X, y)
...
def hypothesis(W: np.matrix, x: np.ndarray, b: np.ndarray) -> np.ndarray:
