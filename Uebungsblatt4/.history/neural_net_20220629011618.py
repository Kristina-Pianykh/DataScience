...
class FeedforwardNetwork:
    ...
    def forward(self, x: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        **Softmax-Funktion** genutzt werden.
        ...
        activated = softmax(not_activated)
    ...
    def predict_labels(self, p: np.ndarray) -> np.ndarray:
        ...
        """
        num = p.shape[0]
        predictions = np.zeros(num, dtype=np.float64)
    ...
    def loss(self, y: np.ndarray, p: np.ndarray) -> np.ndarray:
        ...
        """
        avoid_zero_division = p
        avoid_zero_division[avoid_zero_division == 1] = 1 - EPS
        avoid_zero_division[avoid_zero_division == 0] = EPS
        return -1 * np.dot(y.T, np.log(p))
    ...
    def backward(self, x: np.ndarray, p: np.ndarray, y: np.ndarray):
        ...
        """
        grad_weights = -1 * (np.dot(x.T, (p + (-y)))).T
        grad_bias = -1 * np.sum(p + (-y))
...
def cost_function(actual_labels: np.ndarray, predicted_scores: np.ndarray):
    y_class_arr = np.asarray(actual_labels).reshape(-1)
    y_scores_arr = np.asarray(predicted_scores).reshape(-1)
