...
def main():
    ...
    csv_file = [file for file in getoutput(f"ls {workspace_folder}").split("\n") if file.endswith(".csv")][0]
    png_file = "mein_diagramm.png"
    path_to_read = f"{workspace_folder}/{csv_file}"
    path_to_write = f"{workspace_folder}/{png_file}"
    data: List[Tuple] = read_entries(path_to_read)
    path_to_write = (
